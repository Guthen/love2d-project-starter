...
function Camera:update( dt )
    --  target
    if self.target then
        if self.target.body and not self.target.body:isDestroyed() then 
            self:center( self.target.body:getX(), self.target.body:getY() )
        elseif not self.target.body and self.target.x and self.target.y then
